...
func TestValidateTemplateDir(t *testing.T) {
	t.Run("empty directory", func(t *testing.T) {
		...
		templateDir := filepath.Join(pkgDir, "agent", "input")
		err = os.MkdirAll(templateDir, 0o755)
		require.NoError(t, err)

		fsys := fspath.DirFS(pkgDir)
		err = validateTemplateDir(fsys, filepath.Join("agent", "input"))
		require.NoError(t, err)
	...
	t.Run("valid handlebars file", func(t *testing.T) {
		...
		err = os.MkdirAll(templateDir, 0o755)
		require.NoError(t, err)
		hbsFilePath := filepath.Join(templateDir, "template.hbs")
	...
	t.Run("valid linked handlebars file", func(t *testing.T) {
		...
		pkgDirLinked := filepath.Join(tmpDir, "linked")
		err = os.MkdirAll(pkgDirLinked, 0o755)
		require.NoError(t, err)
		linkedHbsFilePath := filepath.Join(pkgDirLinked, "linked_template.hbs")
		...
		err = os.MkdirAll(templateDir, 0o755)
		require.NoError(t, err)
		hbsFilePath := filepath.Join(templateDir, "template.hbs.link")
