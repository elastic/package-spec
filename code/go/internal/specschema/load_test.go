...
import (
	...
	"github.com/tommyers-elastic/package-spec/v2/code/go/internal/yamlschema"
)
...
func TestPatchedFolderSpec(t *testing.T) {
	cases := []struct {
		title   string
		path    string
		valid   bool
		...
		{
			title:   "folder spec without patches",
			path:    "simple-spec",
			version: semver.MustParse("1.0.0"),
			valid:   true,
		},
		{
			title:   "folder spec with multiple versions 1.0.0",
			path:    "multiple-versions",
			version: semver.MustParse("1.0.0"),
			valid:   true,
		},
		{
			title:   "folder spec with multiple versions 2.0.0",
			path:    "multiple-versions",
			version: semver.MustParse("2.0.0"),
			valid:   true,
		},
		{
			title:   "folder invalid spec patches",
			path:    "invalid-version-patch",
			version: semver.MustParse("3.0.0"),
			valid:   true,
		},
